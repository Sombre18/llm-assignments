...
class LearnedPositionalEmbedding(nn.Module):
    ...
    def __init__(self, max_seq_len: int, embed_dim: int):
        super().__init__()
        # Add 1 to max_seq_len to ensure indices up to max_seq_len are valid
        self.embedding = nn.Embedding(max_seq_len + 1, embed_dim)
...
class SinusoidalPositionalEncoding(nn.Module):
    ...
    def __init__(self, max_seq_len: int, embed_dim: int):
        ...
        position = torch.arange(max_seq_len + 1).unsqueeze(1).float()
        div_term = torch.exp(
